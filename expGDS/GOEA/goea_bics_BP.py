...
# Examples:
#goea_bics_BP.goea_bics_BP('GDS181', './Bics/GDS181logesc_binwidth41', './GAFs/goa_human.gaf')
#goea_bics_BP.goea_bics_BP('GDS750', './Bics/GDS750logesc_binwidth32', './GAFs/sgd.gaf')
def goea_bics_BP(nameData, pathBic, gafFile):

    thr = 0.05 # default significance cut-off
    
    # Reading the biclustering output:
    bics = []
    with open(pathBic + '.py') as bicfile:
        for line in bicfile:
            exec(line)
    # ----
    ...
    ns2assoc = rve_goeaTools.getAssociations(path_gaf=gafFile)
    
    
    gds = rve_goeaTools.dnld_GEO(nameData, './Datasets/')
    ...
    goeaobj.wr_xlsx(pathBic + '_BP_bestP.xlsx', goea_results_sig)
    ...
    goea_results_all = goeaobj.run_study(geneids_study)
    goea_results_sig = [r for r in goea_results_all if r.p_fdr_bh < thr]
    goeaobj.wr_xlsx(pathBic + '_BP_bestN.xlsx', goea_results_sig)
    
    
    print("Results for " + pathBic)
