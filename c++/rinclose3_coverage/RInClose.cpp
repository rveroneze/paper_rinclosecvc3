...
int main(int argc, char* argv[])
{
	...
	// Allocating memory for g_cov
	g_cov = new unsigned*[n];
	for (row_t i = 0; i < n; ++i)
	{
		g_cov[i] = new unsigned[m];
		for (col_t j = 0; j < m; ++j) g_cov[i][j] = 0;
	}
	...
	// Compunting coverage and global overlap
	unsigned long cov = 0, go = 0;
	for (row_t i = 0; i < n; ++i)
	{
		for (col_t j = 0; j < m; ++j)
		{
	...
	cout << "Global Overlap = " << (go - cov) / (float)cov << endl;
