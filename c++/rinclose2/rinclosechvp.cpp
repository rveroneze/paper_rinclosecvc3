...
float runRInCloseCHVP(const dataset_t &D, const row_t &n, const col_t &m, const row_t &minRow, const col_t &minCol)
{
	...
	for (col_t atr = 0; atr <= m - minCol; ++atr)
	{
		...
		bic->sizeB = 1;
		bic->col = atr;
		RInCloseCHVP(D, m, minRow, minCol, bic); // call RIn-Close
	}
...
void RInCloseCHVP(const dataset_t &D, const col_t &m, const row_t &minRow, const col_t &minCol, const pbic_t &bic)
{
			...
			if (bic->sizeA - i >= minRow)
			{
				...
				else if (bic->sizeA > minRow && qnmv >= minRow) // otherwise, bic r can not generate descendants with at least minRow rows
				{
					...
					while (p1 <= qnmv - minRow)
					{
						...
						if (p2 - p1 + 1 >= minRow)
						{
							if (RCHVP_IsCanonical(D, j, p1, p2, bic))
							{
	...
	while (!children.empty())
	{
		...
		RInCloseCHVP(D, m, minRow, minCol, child);
		children.pop();
	}
	delete[] bic->B;
	delete bic;
...
bool RCHVP_IsCanonical(const dataset_t &D, const col_t &y, const row_t &p1, const row_t &p2, const pbic_t &bic)
{
	data_t difpivo;
	row_t i;
	for (col_t j = 0; j < y; ++j)
